...
urls = ['wuhan', 'yichang', 'jinzhou', 'huangshi', 'xiangyang', 'huanggang', 'jinmen', 'xiaogan', 'shiyan',
        'xianning', 'ezhou', 'xiantao', 'suizhou', 'tianmen', 'xianghuan', 'qianjiang', 'shennongjia']  # 湖北省
...
for url in urls:
    ...
    # 中国联通
    url_0 = str(base64.b64encode(f'"Server: udpxy" && city="{url}"'.encode("utf-8")), "utf-8")
...
for urlx in urls_all:
    channel = [f'{name},{url.replace("http://183.92.4.163:4000", urlx)}' for name, url in
               [line.strip().split(',') for line in open("hb-LT.txt", 'r', encoding='utf-8')]]
...
with open("iptv-hb.txt", 'w', encoding='utf-8') as file:
    channel_counters = {}
    ...
    for result in resultxs:
        channel_name, channel_url = result
        if 'CCTV' not in channel_name and '卫视' not in channel_name and '湖北' not in channel_name and '武汉' not in channel_name:
            if channel_name in channel_counters:
                if channel_counters[channel_name] >= result_counter:
                    continue
                else:
