    ...
    for name, url, speed in sources:
        ...
        if any(word.lower() in name.lower() for word in name_dict):
            ...
            name = name.replace("FHD", "").replace("HD", "").replace("hd", "").replace("频道", "").replace("高清", "") \
                ...
                .replace("4kR", "").replace(" ,", ",").replace(" 中文国际 ", "")
...
# 对所有频道列表进行分类排序
def classify_and_sort_sources(region_folder, region_name):
    region_file_path = os.path.join(region_folder, f"{region_name}.txt")
    sorted_file_path = os.path.join(region_folder, f"{region_name}_组播源.txt")
    results = []
...
def main(search_terms):
    ...
    # 对每个地区文件进行分类排序
    for search_term in search_terms:
        classify_and_sort_sources(region_folder, search_term)
