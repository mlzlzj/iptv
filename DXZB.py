...
urls = ["changsha", "zhuzhou", "xiangtan", "hengyang", "shaoyang", "yueyang", "changde", "zhangjiajie", "yiyang",
        "chenzhou", "yongzhou", "huaihua", "loudi"]  # 湖南省
...
def measure_download_speed_parallel(channels, max_threads=5):
    results = []
    queue = Queue()  # 使用 queue 模块的 Queue 类
    processed_count = 0
...
def group_and_sort_channels(channels):
    ...
    for name, url, speed in channels:
        ...
        elif '卫视' in name or '凤凰' in name or 'CHC' in name:
            groups['卫视频道,#genre#'].append((name, url, speed))
        elif '湖南' in name or '长沙' in name or '金鹰' in name:
...
def main():
    ...
    # 分组并排序频道
    groups = group_and_sort_channels(results)
