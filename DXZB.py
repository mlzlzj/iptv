...
for shengshi in shengshi_names:
    ...
    # search_txt = f'\"ZHGXTV\"  && region=\"{shengshi}\"'  
    search_txt = f'\"ZHGXTV\"  && City=\"{shengshi}\"'  
    # 将字符串编码为字节流
...
with open("iptvlist.txt", 'w', encoding='utf-8') as file:
    ...
    # for result in results:
    #     channel_name, channel_url = result.split(',',1)
    #     if 'CCTV' not in channel_name and '卫视' not in channel_name and '测试' not in channel_name and
    #        '湖南' not in channel_name and '长沙' not in channel_name:
    #         if channel_name in channel_counters:
    #             if channel_counters[channel_name] >= result_counter:
    #                 continue
    #             else:
