...
urls = ['changsha', 'zhuzhou', 'hengyang', 'yueyang', 'yiyang', 'huaihua', 'loudi',
        'chenzhou', 'xiangtan', 'changde', 'yongzgou', 'shaoyang', 'zhangjiajie', 'xiangxi']  # 湖南省
...
for urlx in urls_all:
    result = [f'{name},{url.replace("http://8.8.8.8:8", urlx)}' for name, url in
              [line.strip().split(',') for line in channels]]
...
keywords = ['凤凰卫视', '凤凰资讯', 'TVB翡翠', 'TVB明珠', 'TVB星河', 'J2', '无线', '有线', '天映', 'VIU',
            'RTHK', 'HOY', '香港卫视', 'Viu']  # 需要提取的关键字列表
...
keywords = ['民视', '中视', '台视', '华视', '新闻台', '东森', '龙祥', '公视', '三立', '大爱', '年代新闻', '人间卫视',
            '人間', '大立', '八大', 'TVBS', '視','中天','非凡', 'BBC']  # 需要提取的关键字列表
pattern = '|'.join(keywords)  # 创建正则表达式模式，匹配任意一个关键字
...
with open("iptv_list.txt", 'w', encoding='utf-8') as file:
    ...
    for resultx in resultxs:
        channel_name, channel_url = resultx
        if 'CCTV' not in channel_name and '卫视' not in channel_name and '测试' not in channel_name and '湖南' not in channel_name and '长沙' not in channel_name and '金鹰' not in channel_name:
            if channel_name in channel_counters:
                if channel_counters[channel_name] >= result_counter:
                    continue
                else:
